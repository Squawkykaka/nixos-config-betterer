{ types, ... }: {
  ...
  impl =
    ...
    let
      ...
      optPlugins =
        if options ? optPlugins then
          filterAttrs (_: v: v != null && !isTreesitter v) options.optPlugins
        else
          { };
      startPlugins =
        let
      ...
      generatedInitLua =
        let
          ...
          sourceLua = concatStringsSep "\n" (
            map (x: "dofile('${x}')") (
              (
                if options ? initLua then
                  [
          ...
          sourceVimL = concatStringsSep "\n" (
            map (x: "vim.cmd('source ${x}')") (
              (
                if options ? initViml then
                  [
                    (writeText "init.lua" options.initViml)
                  ]
                else
                  [ ]
              )
    ...
    stdenvNoCC.mkDerivation {
      ...
      installPhase = ''
        ...
        ${concatStringsSep "\n" (map (x: ''ln -s "$out/bin/nvim" "$out/bin/"'${x}' '') options.aliases)}
